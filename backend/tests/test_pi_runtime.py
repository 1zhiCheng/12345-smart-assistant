...
async def test_intent_prefers_pi_without_calling_local_llm():
    store = MemoryStore()
    await store.upsert_department({"_id": "dept_city_management", "name": "城市管理局"})
    local = FakeLLM(json_result={"type": "other"})
    pi = FakePi(json_result={
        "type": "deadline_query", "depts": ["dept_city_management"], "user_role": "operator",
        "entities": {"matter": "住房补贴"}, "needs_cross_dept": False, "confidence": 0.9,
    })
    intent = await IntentAgent(local, store, pi, 1.0).infer("住房补贴截止时间", "u1")
...
async def test_answer_and_verifier_use_pi_outputs():
    store = MemoryStore()
    chunk = {
        "_id": "d1:0", "doc_id": "d1", "dept_id": "dept_city_management",
        "chunk_index": 0, "content": "住房补贴截止第八周。", "section_path": [],
    }
    ...
    verdict = await VerifierAgent(local, pi_verify, 3.0).verify("何时住房补贴", answer, [chunk])
