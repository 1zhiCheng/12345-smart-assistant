...
const roles = [
  { key: "operator", username: "operator", password: "operator123", expected: "12345 热线智能受理工作台" },
...
async function run(role, port) {
  const profile=await mkdtemp(join(tmpdir(),`wuhu-12345-${role.key}-`));
  ...
  try {
    ...
    if(role.key!=="operator"){
      const nav=[
        ["态势总览","Loop 渐进退出"],["知识资产","部门与政策事实治理"],
        ["可信审核","审核中心"],["进化 Loop","反馈如何改变下一轮行为"],
        ["记忆与实验","政策事实平面"],["Agent 网络","部门子 Agent 可视化"],
      ];
      for(const [label,expected] of nav){
        ...
        if(role.key==="super"){const tabShot=await call("Page.captureScreenshot",{format:"png",captureBeyondViewport:false});await writeFile(`/tmp/wuhu-12345-super-${label.replace(/\s/g,'-')}.png`,Buffer.from(tabShot.data,"base64"));}
      }
      ...
      await waitFor(call,`document.body.innerText.includes('政策事实平面')`);
    } else {
      ...
      await call("Runtime.evaluate",{expression:`[...document.querySelectorAll('button')].find(b=>b.innerText.includes('录音输入'))?.click()`});
      await waitFor(call,`document.body.innerText.includes('选择脱敏录音')`);checked.push("录音受理");
      await call("Runtime.evaluate",{expression:`[...document.querySelectorAll('button')].find(b=>b.innerText.includes('文本输入'))?.click()`});
      await waitFor(call,`document.body.innerText.includes('群众诉求原文')`);checked.push("文本受理");
    }
    const shot=await call("Page.captureScreenshot",{format:"png",captureBeyondViewport:false});await writeFile(`/tmp/wuhu-12345-${role.key}.png`,Buffer.from(shot.data,"base64"));
    ws.close(); return {role:role.key, title:errors.result.value.title, overflow:false, checked, screenshot:`/tmp/wuhu-12345-${role.key}.png`};
