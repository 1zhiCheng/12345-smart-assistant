...
export default function InsightsPanel({ user }: { user: User }) {
  ...
  return <div className={styles.panelStack}>
    <section className={styles.heroPanel}><div><span className={styles.eyebrow}>MEMORY GOVERNANCE</span><h2>五个记忆平面，一个独立事实平面</h2><p>记忆负责理解上下文与持续学习；政策事实始终回到 active 官方文档与完整 chunk，以来源和版本守住可信边界。</p></div><div className={styles.heroBadge}><Icon name="brain" size={28}/><span><b>{data.memory_planes.reduce((n,p)=>n+p.count,0)}</b><small>可治理记忆单元</small></span></div></section>
    <div className={styles.memoryMap}>
      ...
      <div className={styles.factCore}><div className={styles.orbit}/><span className={styles.factIcon}><Icon name="database" size={28}/></span><small>INDEPENDENT SOURCE OF TRUTH</small><h3>政策事实平面</h3><p>MongoDB 官方文档与 Chunk</p><div className={styles.factStats}><span><b>{f.active_documents}</b>有效文档</span><span><b>{f.chunks}</b>事实切片</span><span><b>{f.relations}</b>知识关系</span><span><b>{f.conflicts}</b>冲突关系</span></div></div>
    </div>
